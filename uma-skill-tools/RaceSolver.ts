...
export interface RaceState {
	...
	readonly distanceTop: number
	readonly isOvertaking: boolean
...
export class RaceSolver {
	...
	// Deterministic multi-agent physics state injected by MultiRaceCoordinator
	currentRank: number = 0;
	distanceTop: number = 0;
	isOvertaking: boolean = false;
	isCompeting: boolean = false;
	distanceDiffRate: number = 0;
	...
	constructor(params: {
		...
		if (StrategyHelpers.strategyMatches(this.horse.strategy, Strategy.Nige)) {
		...
		// removed pacer start delay logic
		this.pos = 0.0;
		this.accel = 0.0;
	...
	initHills() {
		if (!this.course.slopes) (this.course as any).slopes = [];
	...
	updatePositionKeepNonNige() {
		if (this.pos >= this.posKeepEnd) {
			...
			if (
			   this.distanceTop > this.posKeepEffectExitDistance
			...
			|| this.isKakari
			   ...
			   this.distanceTop < this.posKeepMinThreshold
			&& this.activeTargetSpeedSkills.length == 0
			&& this.activeCurrentSpeedSkills.length == 0
			&& !this.isKakari
