...
export function parseGroundCondition(g: string | GroundCondition) {
	...
	case 'GOOD': case 'FIRM': return GroundCondition.Good;
...
export function buildAdjustedStats(baseStats: HorseParameters, course: CourseData, ground: GroundCondition) {
	...
	return Object.freeze({
		speed: Math.max(baseStats.speed * raceCourseModifier + ((GroundSpeedModifier[course.surface] || [])[ground] || 0), 1),
		stamina: baseStats.stamina,
		power: Math.max(baseStats.power + ((GroundPowerModifier[course.surface] || [])[ground] || 0), 1),
		guts: baseStats.guts,
...
export class RaceSolverBuilder {
	...
	// pacer removed for V4 multi-agent topology
	_rng: Rule30CARng
		...
		for (let i = 0; i < this.nsamples; ++i) {
			...
			if (lastskills != null) {
				...
				skills = skilldata.map((sd,sdi) => ({
					...
					let rng = wisdomRngs.get(sd.skillId);
					if (!rng) {
						rng = new Rule30CARng(parseInt(sd.skillId) || 12345, 0);
						wisdomRngs.set(sd.skillId, rng);
					}
					return rng.random() < skillActivationChance[sd.perspective];
				});
			}
